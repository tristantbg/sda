...
kirby()->routes(array(
	array(
		...
		'action'  => function() use ($exclude, $important) {
			...
			foreach(site()->pages()->index()->visible()->filterBy('intendedTemplate', 'not in', ['news', 'post']) as $p){
			...
			foreach(site()->pages()->index()->visible()->filterBy('intendedTemplate', 'in', ['news', 'post'])->filterBy('date', '<', time()) as $p){
				if(!in_array($p->uri(), $exclude)){
					$sitemap .= '<url><loc>' . html($p->url());
					$sitemap .= '</loc><lastmod>' . $p->modified('c') . '</lastmod><priority>';
					$sitemap .= ($p->isHomePage()||in_array($p->uri(), $important)) ? 1 : 0.6/$p->depth();
